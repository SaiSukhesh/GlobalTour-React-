...
export default function TourDetail({ tourId, onBook, navigateTo, nestedPath, setNestedPath }) {
  const tour = PLACES.find(p => p.id === tourId)

  if (tour === undefined) {
    return (
      <div style={{ paddingTop: '120px', textAlign: 'center', minHeight: '100vh' }}>
        <h3>Destination details route configuration mismatch.</h3>
        <button className="btn" onClick={() => navigateTo('/explore')}>Return to Explore</button>
      </div>
    )
  }
  ...
  return (
    <div style={{ paddingTop: '100px', paddingBottom: '60px', minHeight: '100vh', maxWidth: '800px', margin: '0 auto', paddingLeft: '20px', paddingRight: '20px' }}>
      <a onClick={() => navigateTo('/explore')} style={{ cursor: 'pointer', color: 'rgb(15, 226, 142)', fontWeight: 'bold', textDecoration: 'none' }}>← Back to Explore</a>
